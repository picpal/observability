...
web 계층(nginx)은 `stub_status` + nginx-prometheus-exporter 로 같은 Prometheus 에 수집한다.

**Prometheus/Grafana 의 실체** = 온프렘 폐쇄망 **Monitoring VM 1대에 `docker-compose.prod.yml`** 로 구축.
이미지 반입(docker save/load)·설치·App 서버 exporter·방화벽까지 설치 전 과정은
[`install-airgap.md`](install-airgap.md) 참조. 본 문서는 메트릭·대시보드·알람·앱 설정 등 *설계와 운영* 을 다룬다.
...
### 4.3 web 계층 메트릭 (nginx, `job="nginx"`)

WAS 앞단 nginx 의 연결/요청 메트릭. nginx 의 `stub_status` 를 nginx-prometheus-exporter(9113)가
읽어 노출한다. OSS nginx 기준이라 stub_status 가 제공하는 범위(연결/요청 카운터)로 한정된다.
설치는 [`install-airgap.md`](install-airgap.md) §5, 대시보드는 `grafana/dashboards/nginx.json`.

| 메트릭 | 타입 | 의미 |
|---|---|---|
| `nginx_up` | Gauge | exporter 가 stub_status 를 읽었는지 (1/0) |
| `nginx_http_requests_total` | Counter | 누적 요청 수 → `rate()` 로 req/s |
| `nginx_connections_active` | Gauge | 현재 활성 연결 |
| `nginx_connections_reading`/`writing`/`waiting` | Gauge | 연결 상태별 분해 |
| `nginx_connections_accepted`/`handled` | Counter | accepted − handled = drop (평소 0, 양수면 연결 한계) |

`instance` 라벨은 message-gate WAS 와 동일하게 `was-1`/`was-2` 로 relabel 되어 한 화면에서 짝지어 본다.
  ...
  - name: message-gate
    ...
    rules:
      ...
      - alert: PrometheusScrapeDown
        expr: up{job=~"message-gate|nginx"} == 0      # WAS actuator + nginx exporter 모두 감시
        for: 2m
        labels: { severity: critical }
        annotations:
          summary: "{{ $labels.job }} / {{ $labels.instance }} 메트릭 수집 중단"
```
...
- `security-review.md` — 폐쇄망 반입 보안심의 근거 자료 (방화벽 명세, 잔여위험)
